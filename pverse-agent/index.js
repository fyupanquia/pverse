...
const debug = require("debug")("pverse:agent");
const mqtt = require("mqtt");
const defaults = require("defaults");
const uuid = require("uuid");
const EventEmitter = require("events");

const { json } = require('pverse-utils')
...
class PAgent extends EventEmitter {
  constructor(opts) {
    super();
    ...
    this._client = null;
    this._agentId = null;
    console.log(json.parsePayload('{"name":"frank"}'));
  }
  ...
  connect() {
    if (!this._started) {
      ...
      this._client.on("message", (topic, payload) => {
        payload = json.parsePayload(payload);
